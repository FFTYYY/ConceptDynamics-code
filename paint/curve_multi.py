...
def get_data(exp_id: str, device: str):
    cache_path = f"paint/cache/curve_multi_{exp_id}.pkl"
    if os.path.exists(cache_path):
        ...
        for epoch_idx in tqdm(checkpoints):
            ...
            pred_Ys.append(m_pred_Y.to(device))
...
def paint(idx:int, exp_id: str, label: str, device: str):
    ...
    xx = np.array([_[0] for _ in pred_Ys])
    yy = np.array([_[1] for _ in pred_Ys])
    ...
    for pos in range(0, len(xx) - 1, 2):
        ...
        plt.annotate(
            ...
            arrowprops=dict(arrowstyle='-|>', color=color, lw=0, alpha=0.7, mutation_scale=25)
        )
    # plt.plot(xx, yy, color = color, linewidth = 4, marker="<", markersize = 12, alpha=0.7)
...
def main(C: MyDict):
    ...
    # else: 
    #    plt.xticks([0.0, 1.0, 2.0], fontsize=14)
    #    plt.yticks([0.0, 2.0, 4.0], fontsize=14)
    ...
    out_filename = f"figure/{prefix}curve/{','.join(exp_ids)}.pdf"
